...
import { PLAYER_STATS } from '../src/data/players.js'
import { TEAMS } from '../src/data/teams.js'
import { HISTORY } from '../src/data/history.js'
import { FIXTURES as FROZEN_FIXTURES } from './fixtures/frozen/fixtures.js'
import { PLAYER_STATS as FROZEN_PLAYER_STATS } from './fixtures/frozen/players.js'
import { TEAMS as FROZEN_TEAMS } from './fixtures/frozen/teams.js'
import { HISTORY as FROZEN_HISTORY } from './fixtures/frozen/history.js'
...
describe('generated data', () => {
  ...
  it('carries the do-not-edit banner and names its builder', () => {
    for (const file of ['src/data/fixtures.js', 'src/data/history.js', 'src/data/players.js', 'src/data/teams.js']) {
      const src = read(file)
...
describe('frozen data', () => {
  // The 100% gate must not move when a refresh rewrites src/data. On the WNBA sibling it
  // did three times in six weeks, and on the NFL sibling the first week-1 refresh went
  // red, each time because some test still reached a live module, directly or through
  // src/. The frozenData plugin in vite.config.js ends that by resolving every such
  // import to a stand-in. These keep it true.

  // Identity, not equality: the same object means the import was redirected, not that
  // the live data happens to match today.
  it('is what an import of a live data module actually receives', () => {
    expect(FIXTURES).toBe(FROZEN_FIXTURES)
    expect(PLAYER_STATS).toBe(FROZEN_PLAYER_STATS)
    expect(TEAMS).toBe(FROZEN_TEAMS)
    expect(HISTORY).toBe(FROZEN_HISTORY)
  })

  // The shape of the mistake: a fetch script starts writing another module and nobody
  // freezes it, so the suite quietly reads live data again. These are the three scripts
  // `npm run fetch:all` runs, which is what the refresh workflow runs.
  it('covers every module the refresh rewrites', () => {
    const fetchers = ['scripts/fetch-fixtures.mjs', 'scripts/fetch-stats.mjs', 'scripts/fetch-history.mjs']
    expect(JSON.parse(read('package.json')).scripts['fetch:all']).toBe(
      'npm run fetch:fixtures && npm run fetch:stats && npm run fetch:history'
    )
    const written = fetchers.flatMap((f) =>
      [...read(f).matchAll(/['"](src\/data\/\w+\.js)['"]/g)].map((m) => m[1])
    )
